      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        <div className={`absolute inset-0 flex flex-col items-center justify-center text-center px-6 transition-all duration-1000 ${activeSvc === -1 ? 'opacity-100 scale-100' : 'opacity-0 scale-105 pointer-events-none'}`}>
          ...
          <div className="relative z-10 w-full max-w-7xl mx-auto text-center">
            <h2 className="text-7xl md:text-[12rem] font-black text-white uppercase tracking-tighter leading-[0.8] mb-12 reveal-on-scroll">
              Nuestros <br />
              <span className="text-outline-white">Servicios</span>
            </h2>
            <div className="w-40 h-2 bg-accent/60 mx-auto mb-12 reveal-on-scroll delay-100" />
        ...
        {[0, 1, 2].map((step) => {
          ...
          return (
            <div
              key={step}
              className={`absolute inset-0 z-10 p-6 md:p-24 transition-all duration-1000 flex flex-col items-center justify-center ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20 pointer-events-none'}`}
            >
              ...
              <div className="relative z-10 w-full max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20">
                {items.map((svc, idx) => (
                  <div
                    key={`${step}-${idx}`}
                    className="group relative aspect-[3.4/4] rounded-[3rem] bg-[#111111] overflow-hidden transition-all duration-700 hover:shadow-[0_40px_100px_rgba(0,0,0,0.8)]"
                  >
                    <div className="absolute inset-0 w-full h-full overflow-hidden">
                      <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent opacity-80 z-[5]" />
                      <img
                        src={svc.image}
                        alt={svc.title}
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-10 z-10 bg-black/40 backdrop-blur-2xl border-t border-white/5">
                      <div className="flex items-center gap-3 mb-4 reveal-on-scroll">
                        <h3 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tight font-paloseco">{svc.title}</h3>
                        <div className="w-5 h-5 md:w-6 md:h-6 bg-blue-500 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(59,130,246,0.5)]">
                          <svg viewBox="0 0 24 24" className="w-3 h-3 md:w-4 md:h-4 text-white fill-current"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z" /></svg>
                        </div>
                      </div>
                      <p className="text-white/60 text-base md:text-lg leading-relaxed font-sans group-hover:text-white/90 transition-colors reveal-on-scroll delay-100">{svc.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
